			...
			if (confirmReplace) {
				...
				personService
					...
					.then(response => {
						...
						setNotification({ message: `Updated number for ${response.data.name}`, type: 'success' })
    					setTimeout(() => setNotification({ message: null, type: null }), 5000)
					})
					.catch(error => {
        				setNotification({ message: `Error: '${existingPerson.name}' was already removed from server`, type: 'error' })
